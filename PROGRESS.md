...
- [ ] **P0 — 本地跑通**：填 `.env`，`docker compose up`，端到端验证一次对话
- [ ] **P0 — Bug 修复**：骨架代码未经测试，预计有若干小问题（见下方已知问题）
- [ ] **P1 — 流式输出**：目前 `agent/core.py` 是整块返回，改成真正 token 级别 SSE 流式
- [ ] **P1 — 对话摘要压缩**：历史超过 `MAX_HISTORY_TOKENS` 时自动摘要，防止 context 爆炸
